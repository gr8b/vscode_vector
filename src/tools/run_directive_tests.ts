...
const tests: DirectiveTestCase[] = [
    ...
    },
    {
        ...
        expect: {
            ...
            // LXI H, CONST1 + >CONST2 = 0x21, 0xA0, 0x40
            bytes: [0xC6, 0x12, 0xD6, 0x34, 0xE6, 0xCD, 0xF6, 0xAB, 0xFE, 0x35, 0xDB, 0x00, 0xD3, 0x12, 0x21, 0xA0, 0x40],
            noWarnings: true
        },
    },
    {
        name: 'Immediate opcodes evaluate expressions consistently',
        sourceFile: 'immediate_expression_support.asm',
        expect: {
            bytes: [
                0x3E, 0x35, // MVI A, (<ADDR + 1)
                0x21, 0x32, 0x40, // LXI H, CONST1 + >ADDR
                0x32, 0xA0, 0x40, // STA
                0x3A, 0x54, 0x40, // LDA
                0x22, 0xA0, 0x40, // SHLD
                0x2A, 0x54, 0x40, // LHLD
                ...
                0xF4, 0x32, 0x40, // CP
                0xFC, 0x32, 0x40, // CM
                0xD3, 0x13, // OUT (>ADDR + 1)
                0xDB, 0x36, // IN (<ADDR + 2)
                0xC6, 0x37, // ADI (<ADDR + 3)
                0xD6, 0x38, // SUI (<ADDR + 4)
                0xE6, 0x13, // ANI (>ADDR + 1)
                0xF6, 0x39, // ORI (<ADDR + 5)
                0xCE, 0x14, // ACI (>ADDR + 2)
                0xDE, 0x3A, // SBI (<ADDR + 6)
                0xEE, 0x15, // XRI (>ADDR + 3)
                0xFE, 0x3B  // CPI (<ADDR + 7)
            ],
            noWarnings: true
        }
    },
